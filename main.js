...
const movedown = (now) => {
  if (!gameObject.onPlatform && !gameObject.onPrize) {
    ...
    mario.style.bottom = prizePlatform.getBoundingClientRect().top + 120 + "px";
  }
};
...
const breatheFire = () => {
  if (gameObject.bowserShooting) {
    ...
    if (fireCollisionCheck() === true) {
      ...
      fireBall.getBoundingClientRect().left < game.getBoundingClientRect().left
    ) {
      fireLeft = 0;
      bowserFireCounter = 0;
      fireBall.remove();
...
const moveGoomba = () => {
  ...
  if (goombaCollisionCheck() === true) {
    ...
    goomba.getBoundingClientRect().left < game.getBoundingClientRect().left &&
...
const onplatformCheck = () => {
  if (
    ...
    mario.getBoundingClientRect().right <=
      platform.getBoundingClientRect().right &&
    mario.getBoundingClientRect().bottom < platform.getBoundingClientRect().top
  ) {
    gameObject.onPlatform = true;
